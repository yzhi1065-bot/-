...
from app.core.permissions import require_permissions, PATIENT_READ
...
def list_constitutions(current_user: User = Depends(require_permissions(PATIENT_READ))):
    """获取所有体质类型"""
    return Response(data=get_constitution_list())


@router.post("/analyze", response_model=Response)
def analyze(request: dict, current_user: User = Depends(require_permissions(PATIENT_READ))):
    """分析体质（接收症状字典）"""
